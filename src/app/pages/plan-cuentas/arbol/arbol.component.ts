...
export class ArbolComponent implements OnInit, OnChanges {
  ...
  loadTreeRubros() {
    const getters: NbGetters<
      EstructuraArbolRubrosApropiaciones,
      EstructuraArbolRubrosApropiaciones
    > = {
      dataGetter: (node: EstructuraArbolRubrosApropiaciones) =>
        node.data || null,
      childrenGetter: (node: EstructuraArbolRubrosApropiaciones) =>
      ...
      if (raiz[0].children != null) {
        raiz[0].children.forEach((element, index) => {
          ...
          raiz[0].children[index].Hijos.forEach((element2, index2) => {
            ...
            raiz[0].children[index] = Object.assign(
              raiz[0].children[index],
              childrenData2
            );
            contador += 1;
          });
